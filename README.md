...
that it will accompany the "starter" repository that an instructor shares with a
...
will check to ensure that `ComputeMoonDistance` produces the correct number of
lines of output and that you use the `new Date()` construct in the Java code.
GatorGrader will also check that your program performs the correct computation
and produces the expected output for the number of yards to the moon. Please
note that the provided version of `ComputeMoonDistance.java` will not compile
correctly because it does not follow all of the syntax rules of the Java
programming language. It is also important to point out that the program is also
semantically incorrect and thus it will not, by default, perform the correct
computation.
...
session. Please see the course instructor or one of the student technical
...
```
git remote add download git@github.com:Allegheny-Computer-Science-100-F2019/cs100-F2019-practical3-starter.git
```
...
3](https://github.com/Allegheny-Computer-Science-100-F2019/cs100-F2019-practical3-starter)
repository and [raise an
issue](https://github.com/Allegheny-Computer-Science-100-F2019/cs100-F2019-practical3-starter/issues).
